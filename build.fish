...
podman run --rm --arch $architecture --volume $mountpoint:/mnt:Z registry.fedoraproject.org/fedora:latest \
    bash -c "dnf -y install --installroot /mnt --releasever 34 glibc-minimal-langpack openocd --nodocs --setopt install_weak_deps=False"
or exit
...
buildah config --entrypoint '["openocd"]' $container
or exit
